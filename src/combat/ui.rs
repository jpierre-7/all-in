    ...
    root.with_children(|root| {
        ...
        .with_children(|mid| {
            ...
            if duel.phase() == Phase::PushYourLuck {
                ...
                text(mid, format!("{} against {}. Push Your Luck?   Hold for a Payout of {held}, or {pushed} if the coin lands your way.", duel.hand(), duel.house_edge()), 22.0, GOLD);
                text(mid, format!("Lose and The Hand is 0: a Whiff of {}, out of your own Stack.", duel.house_edge()), 16.0, NEON);
                text(mid, coin_line(duel.coin()), 16.0, DIM);
            }
...
fn turn_line(turn: &TurnResult) -> String {
    ...
    match (turn.pyl, turn.kind) {
        ...
        (_, Outcome::Payout(0)) => {
            format!("{} against {edge}. Dead even: nobody pays.", turn.hand)
        }
        (_, Outcome::Payout(n)) => format!("{} against {edge}: Payout {n}.", turn.hand),
        (_, Outcome::Whiff(n)) => {
